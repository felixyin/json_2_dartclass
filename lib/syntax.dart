...
class TypeDefinition {
  ...
  String getFileName(String s) {
    ...
    List<String> list =
        re.allMatches(s).map((v) => v.group(0).toLowerCase()).toList();
    list.removeWhere((v) => v.isEmpty);
    return list.join('_');
  }

  String importExpression(String key, bool privateField) {
    return _importList.join('');
  }
  ...
  String toJsonExpression(String key, bool privateField) {
    ...
    if (isPrimitive) {
      ...
      // class list
      _importList.add("import  '${getFileName(key)}.dart';\n");
  ...
  List<String> _importList = List<String>();
}
...
class ClassDefinition {
  ...
  String get _importFunc {
    final sb = new StringBuffer();
    fields.keys.forEach((k) {
      String s = fields[k].importExpression(k, privateFields);
      if(s.isNotEmpty){
        sb.write(s);
      }
    });
